...
> Evidence checked to establish an authenticated identity.

## Location

### Folder
- must use `Authenticators/` for a role folder under the domain that owns the capability.
...
- must start a concrete implementation with **Authenticates**, naming the identity established from the evidence.
...
### Type name
- must suffix with `Authenticator`.
