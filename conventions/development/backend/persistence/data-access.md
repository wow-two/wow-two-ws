...
- `AddEnumTypeHandler<TEnum>` (constraint `TEnum : struct, Enum`) registers `EnumTypeHandler<TEnum>`: writes emit the chosen `CaseStyle`, reads are
  case-insensitive.
- For Postgres **native** enum types, use Npgsql's driver-level `MapEnum` instead — see [enums.md](enums.md).
...
- **No DbContext** — Dapper uses raw connections; if you need EF, use an EF repository against the `DbContext` instead — see [database.md](database.md).
