# Historical backend build context

Archived 2026-09-15 during handoff cleanup. The [active context](context.md)
...
- Full backend convention audit: [conventions-audit.md](conventions-audit.md), originally 135 docs;
  the final naming acceptance reviews the expanded tree in [naming-final-acceptance.md](naming-final-acceptance.md).
- Convention sweep complete: all 25 `BC01`–`BC25` tasks closed. Workspace task `con-t-003` is done.
- Evidence: [conventions-resolution.md](conventions-resolution.md). All eight design points and both naming
  discussion rows are settled. P01: sealed records and original-instance tracked writes. P02: external validation.
  P03: explicit equality/hashing where needed. P04: inherited docs with scoped overrides. P05: request-file mapping.
  P06: shared services without nested dispatch. P07: optional clarifying test comments. P08: shared stored JSON
  with custom options where needed; no per-type wrapper/holder. SDK N100/N101 implementation remains open.
...
- P01 evidence: [entity records](entity-record-analysis.md), ten BCL checks and twenty EF/SQLite checks. Records work for normal tracking and detached updates; default navigation equality/hash risks reproduced. The tracked-write convention is settled; no SDK source changed.
- Order: settle conventions → finish SDK sweep → verify and publish the new SDK version.
- Final acceptance includes all naming definitions: 159 backend docs, 1,050 local links/fragments pass.
  [Evidence](naming-final-acceptance.md). SDK runtime and release checks remain separate.
- SDK tracker: 107 rows, 73 marked complete (72 historical plus convention-only N25), 6 refuted, 28 open. `C14`–`C29` remain; `N114` applies the settled record/tracked-write convention, with no class conversion or implicit copy merge. `N60` remains open against its static factory. Repeated `N94`–`N98` IDs require subject-qualified references.
- P08: pass options or select a registered profile through the shared serializer. C29 completes the shared
  capability; no general keyed JSON-options registry was found in the earlier source inventory.
- SDK naming inventory: [current dispositions and source evidence](sdk-naming-inventory.md).
  [Permission recovery](permission-recovery.md) resolved test-socket and NuGet blocks through native approval.
  Shared recovery instructions are in `/Users/max/.codex/AGENTS.md`; security settings remain unchanged.
- Deep copying: FastCloner 3.5.6 verified on .NET 10.0.8 Arm64; all 32 graph-copy checks passed, including the hash-key lookups that failed in the DeepCloner control. [Library analysis](deep-copy-analysis.md) and retained results updated. C26 SDK integration remains outstanding; no SDK cloning implementation is claimed.
- Developer owns the SDK; no production consumers; breaking changes permitted.
- Recorder/mapper/policy affected builds pass with existing warnings; 28 scoped tests passed. No full-suite, pack or release result is claimed. The older implementation snapshots below remain historical.

---

## Naming handoff — 2026-09-15

- Remaining coordinators applied as `SecondLevelRetryService` and `SagaService<TState>`;
  [verification](coordinator-service-verification.md): scoped builds passed and 17 existing tests passed.
- `EventSagaService` / `IEventSagaService` applied; isolated core compilation passed. Summary corrected to
  one scope per execution. [Verification](event-saga-service-verification.md): no direct runtime test exists.
- `NoOpMigrationRunnerService` applied; Testing.Data build passed. No pending builds or approvals.
- Exporter retained; convention and indexes added for structured-data exchange documents (CSV/XLSX in the SDK).
  SDK placement/documentation conformance remains tracked by N100.
- Formatter retained; definition and construct/behavior index entries added for values expressed as display
  text under culture/format rules. SDK placement/documentation conformance remains in N100.
- `HumanizedTextFormatter` / `IHumanizedTextFormatter` applied under Formatter; both formatter pairs moved to
  Localization/Formatters. Core build passed; no existing direct tests. [Evidence](humanized-text-formatter-verification.md).
- N100/N101 vocabulary decisions closed; SDK implementation/conformance remains open.
- Transport retained for sending/receiving through a selected medium; definition and indexes added.
  Source conformance remains, including the saga wrapper that calls IEventBus rather than owning a medium.
- Serializer retained; definition and indexes added for object data to/from a specified encoded representation.
  Earlier rejection of per-type stored-JSON wrappers remains in force. Source conformance remains in N101.
- GoogleIdTokenAuthenticator and HashChainValidator source renames applied; core builds passed, no direct
  existing tests. Reports: [Google](google-id-token-authenticator-verification.md), [chain](hash-chain-validator-verification.md).
- Bus retained for application-facing publish/send over a Transport; definition and indexes added.
- Guest session Service rename applied; Identity.Tests build and one existing registration test passed.
  [Evidence](guest-session-service-verification.md).
- MetricsService applied; Messaging.Tests compiled and four existing pump tests passed.
  [Evidence](messaging-metrics-service-verification.md).
- Envelope naming confirmed: EventEnvelopeModel/OtpDeliveryEnvelopeModel internally, TestApiResponse<T> for
  the existing data-only HTTP test mirror. Events retain Event, not EventModel. Renames applied; Messaging.Tests
  and Testing compiled, eight existing claim-check tests passed. [Evidence](envelope-role-verification.md).
- All naming decisions and final convention acceptance closed; the SDK implementation sweep is unblocked.
  SDK naming rows remain open for source conformance; no SDK/release completion is claimed.
- Follow-ups retained for SDK sweep: hash-chain segment validation docs require an unsupported starting
  checkpoint; Google token cancellation is not forwarded; Google audience options use a get-only collection.
